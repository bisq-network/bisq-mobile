...
6. **Follow the MVIP pattern**: sealed `*UiAction` + immutable `*UiState` handled by the presenter; implement the composable as stateless (no new `I*Presenter` interfaces)
