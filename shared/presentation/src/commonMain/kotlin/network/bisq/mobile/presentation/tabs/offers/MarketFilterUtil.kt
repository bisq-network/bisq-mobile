    ...
    internal fun sortMarketsStandard(markets: List<MarketListItem>): List<MarketListItem> {
        return markets.sortedWith(
            compareByDescending<MarketListItem> { it.numOffers }
                .thenByDescending { OffersServiceFacade.Companion.mainCurrencies.contains(it.market.quoteCurrencyCode.lowercase()) }
                .thenBy { item ->
                    if (!OffersServiceFacade.Companion.mainCurrencies.contains(item.market.quoteCurrencyCode.lowercase())) {
