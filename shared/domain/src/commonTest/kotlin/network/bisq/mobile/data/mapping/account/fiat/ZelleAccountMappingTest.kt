...
class ZelleAccountMappingTest {
    ...
    @Test
    fun `toDto maps all ZelleAccount fields correctly`() {
        val domain =
            ZelleAccount(
                accountName = "Zelle Main",
                accountPayload =
        ...
        assertEquals("Zelle", dto.accountPayload.paymentMethodName)
        assertEquals("USD", dto.accountPayload.currency)
        assertEquals("United States", dto.accountPayload.country)
        assertEquals("2026-05-10", dto.creationDate)
        assertEquals("1000.00", dto.tradeLimitInfo)
        assertEquals("8 days", dto.tradeDuration)
    }
