...
class WiseAccountMappingTest {
    ...
    fun `toDomain maps all WiseAccountDto fields correctly`() {
        ...
        assertEquals(listOf("USD", "EUR"), domain.accountPayload.selectedCurrencyCodes)
    ...
    @Test
    fun `toDto maps all WiseAccount fields correctly`() {
        val domain =
            WiseAccount(
                accountName = "Wise Main",
                accountPayload =
        ...
        assertEquals("Satoshi Nakamoto", dto.accountPayload.holderName)
        assertEquals("satoshi@example.com", dto.accountPayload.email)
        assertEquals(FiatPaymentMethodChargebackRiskDto.MODERATE, dto.accountPayload.chargebackRisk)
        assertEquals("Wise", dto.accountPayload.paymentMethodName)
        assertEquals("2026-05-10", dto.creationDate)
        assertEquals("5000.00", dto.tradeLimitInfo)
        assertEquals("4 days", dto.tradeDuration)
    }
    ...
    fun `toDto maps create WiseAccount fields correctly`() {
        val domain =
            CreateWiseAccount(
                accountName = "Wise Main",
                accountPayload = CreateWiseAccountPayload(selectedCurrencyCodes = listOf("USD", "EUR"), holderName = "Alice", email = "alice@example.com"),
            )
