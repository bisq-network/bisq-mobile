...
data class HttpClientSettings(
    val apiUrl: String?,
    val proxyUrl: String?,
    val isExternalProxyTor: Boolean?,
) {
    companion object {
        fun from(settings: Settings?) =
            HttpClientSettings(settings?.bisqApiUrl, settings?.proxyUrl, settings?.isExternalProxyTor)
    }

    fun bisqProxyConfig(): BisqProxyConfig? {
        // isExternalProxyTor is for future non tor proxy support
        if (this.proxyUrl?.isBlank() == false && this.isExternalProxyTor == true) {
